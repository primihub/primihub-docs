...
```bash
./primihub-cli --task_type=2 --params="queryIndeies:STRING:0:11,serverData:STRING:0:pir_server_data,databaseSize:STRING:0:569,outputFullFilename:STRING:0:./data/result/pir5.csv" --input_datasets="serverData"
```
...
| params.databaseSize | STRING | 569 | 表示检索的pir数据库的数据记录数量，当前版本pir需要从cli通知客户端节点数据库的规模，如果未提供该参数或者该参数设置错误则会引发pir报错。 |
| params.outputFullFilename | STRING | ./data/result/pir5.csv | 指定pir匿踪查询结果保存文件的文件名和文件所在目录的路径。|
